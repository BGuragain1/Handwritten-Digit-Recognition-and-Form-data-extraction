...
def pre_process_img(img):
  ...
  final_img = addBlackSurface(dilated_image)
  resized_image = cv2.resize(final_img , (28, 28))
...
def seperate_words(img):
  ...
  max_area = 1000  # Adjust based on expected word size (pixels)
...
def addBlackSurface(img):
  h, w = img.shape

  # Define the size of the new image with black surface
  new_h = h + 7  # Add some padding (adjust as needed)
  new_w = w + 7  # Add some padding (adjust as needed)
